...
function Start() {
	...
	if(Network.peerType == NetworkPeerType.Disconnected) Network.InitializeServer(9, 2500); //We are running in the editor, and have started the game in world load mode
...
function Init() {
	...
	networkView.RPC("iV", RPCMode.All, Network.AllocateViewID(), Settings.networkMode, GameVehicleID, (i != 1 ? temp + " " + i : temp), 0, (isHost ? 1 : 0), 0, 0);
	...
	if(isHost) {
		...
		if(whirldIn.worldParams["message"]) {
			...
			networkView.RPC("msg", RPCMode.All, Settings.serverWelcome, parseInt(chatOrigins.Remote));
		}
		if(whirldIn.worldParams["marsxplr"]) networkView.RPC("sSS", RPCMode.All, whirldIn.worldParams["marsxplr"]);
	}
	else {
...
function Update() {
	...
	if(Settings.resetTime > 0 && (10 - (Time.time - Settings.resetTime)) < 1) {
		...
		Game.Player.rigidbody.isKinematic = false;
		Game.Player.rigidbody.velocity = Vector3.zero;
	...
	if(Network.peerType != NetworkPeerType.Disconnected && !isHost && WorldDesc.url == "" && Time.time - authUpdateTime > 1 && Time.timeSinceLevelLoad > 3) {
		networkView.RPC("lMI", RPCMode.Others, Network.player, GameData.userName); //Sent to all others (and not just server) as we want the "pseudohost" of dedicated servers to be able to let us in if necessary
...
//				}
	...
	else if(Screen.lockCursor) GUI.Button(Rect((Screen.width * 0.5) - 200,Screen.height - 50,400,20), "{Cursor Locked ~ " + (Input.GetButton("Fire2") ? "Release \"Alt\" to unlock" : (Input.GetButton("Snipe") ? "Release \"Shift\" to unlock" : "Press \"2\" to unlock")) + "}", hudTextStyle);
...
function OnPlayerConnected(player : NetworkPlayer) {
	...
	for(var plrE : DictionaryEntry in Players) {
		...
		networkView.RPC("iV", player, veh.networkView.viewID, veh.networkMode, veh.vehId, veh.gameObject.name, (veh.isBot ? 1 : 0), veh.isIt, veh.score, (veh.specialInput ? 1 : 0));
		veh.networkView.RPC("sC", player, veh.vehicleColor.r, veh.vehicleColor.g, veh.vehicleColor.b, veh.vehicleAccent.r, veh.vehicleAccent.g, veh.vehicleAccent.b);
	}
	...
	//Welcome them!
	if(Settings.serverWelcome != "") networkView.RPC("msg", player, Settings.serverWelcome, parseInt(chatOrigins.Remote));
...
function WindowServerSetup (id : int) {
	if(!isHost && WorldDesc.url == "") {
		...
		if(GUILayout.Button("<< Cancel", GUILayout.Height(25), GUILayout.Width(150))) {
			if (!whirldIn.usingCache) //(we're busy storing it, but it's only half-finished D:)
				whirldIn.CacheClear(WorldDesc.url);
			netKillMode = 1;
			Network.Disconnect();
		}
		GUILayout.FlexibleSpace();
	...
	else if(whirldIn.status == WhirldIn.WhirldInStatus.Success || worldLoaded == true) {
		if(whirldIn.status == WhirldIn.WhirldInStatus.Success && !worldLoaded) {
			worldLoaded = true;
			Settings.simplified = false;
	...
	else if(whirldIn.status != WhirldIn.WhirldInStatus.Idle) {
		if(whirldIn.status == WhirldIn.WhirldInStatus.Working) {
		...
		GUILayout.BeginHorizontal();
		GUILayout.Space(40);
		if(GUILayout.Button((whirldIn.status == WhirldIn.WhirldInStatus.Working ? "<< Cancel" : "<< Retry"), GUILayout.Height(40))) {
			whirldIn.Cleanup();
			whirldIn = null;
			whirldIn = new WhirldIn();
		}
		...
		if(hostPanelTab == 0) {
			/*if(WorldIsCustom) {
				...
				GUILayout.BeginHorizontal();
				if (!GameData.gameWorlds.length) {
					i = 0;
					WhirldIn.CacheList();
					for (var cachedWhirld : String in WhirldIn.cacheList) {
						var Aworld = new GameWorldDesc("cachedWhirld"+i, "file://"+cachedWhirld, true);
						if(!WorldIsCustom && WorldDesc && WorldDesc.name == Aworld.name) GUILayout.Toggle(true, Aworld.name, /*GUILayout.Width(170),*/ GUILayout.Height(22));
				        else if(GUILayout.Toggle(false, Aworld.name, GUILayout.Width(170), GUILayout.Height(22))) {
							WorldIsCustom = false;
							WorldDesc.name = Aworld.name;
							WorldDesc.url = Aworld.url;
					...
					for (var Gworld : GameWorldDesc in GameData.gameWorlds) {
						...
						if(Gworld.name[Gworld.name.length-1] != '*')
							if(WhirldIn.CacheCheck(Gworld.url)) {
								Gworld.name = Gworld.name+"*";
							}
...
function addBot() {
	...
	networkView.RPC("iV", RPCMode.All, Network.AllocateViewID(), Settings.networkMode, GameVehicleID, temp, 1, 0, 0, 0);
...
function setVeh(setVehicleTo : int) {
	...
	networkView.RPC("iV", RPCMode.All, Network.AllocateViewID(), Settings.networkMode, GameVehicleID, name, 0, isIt, score, specialInput);
}
...
function iV(viewID : NetworkViewID, networkMode : int, vehId : int, vehName : String, isBot : int, isIt : int, score : int, specialInput : int) {
	...
	//Create Vehicle
	if(networkMode == 0) objectVehicle.networkView.stateSynchronization = NetworkStateSynchronization.Unreliable;
	else if(networkMode == 1) objectVehicle.networkView.stateSynchronization = NetworkStateSynchronization.ReliableDeltaCompressed;
	else objectVehicle.networkView.stateSynchronization = NetworkStateSynchronization.Off;
...
function sSHS() {
	networkView.RPC("sSH", RPCMode.Server,
		serverName,
...
//	if (WorldDesc.url == "http://papawheely.com/Freestyle/Whirld.utw")
//	{

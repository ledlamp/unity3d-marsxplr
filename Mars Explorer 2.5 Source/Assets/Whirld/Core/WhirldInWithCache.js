...
class WhirldInCache extends System.Object {
...
var readChr = 0;
var usingCache = true;
static var cacheList : String[];
static var cacheSize : int;
#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN
static var cachePath : String = (Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor ? System.Environment.GetFolderPath (System.Environment.SpecialFolder.LocalApplicationData) + "//WhirldCache//" : System.Environment.GetFolderPath (System.Environment.SpecialFolder.Desktop) + "//..//WhirldCache//");
#else
...
function Generate() {
	...
	var allRenderers : Renderer[] = monoBehaviour.FindObjectsOfType(Renderer) as Renderer[];
	for (var rendrr in allRenderers) {
		for (var mat in rendrr.materials) {
...
function ReadObject(parent : Transform) {
	...
	while(true) {
		...
		//Assign char to property we are reading
			...
			// These couple of lines are incorrect. uncomment the if and else clause, and it won't work.
			// s must be added to v. not n.
			// - ACE
...
static function GetCacheURL(url : String) {
	url = url.Replace("http://","");
	var urls : String[] = url.Replace("https://","").Split("?"[0]);
	url = (cachePath + urls[0]);
	return url;
}

function CheckLocalURL(url : String) {
	//Handle local file://s
	url = url.Replace("\\", "/");
	if(url.StartsWith("file:///")) return url;
	else if(url.StartsWith("file://")) return url.Replace("file://", "file:///");
	//if (!url.StartsWith("http")) return "file:///" + url;
	else return "";
}

function Cache(url : String) : String {
	#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_EDITOR_OSX
	
	var localCheck = CheckLocalURL(url);
	if(localCheck != "") return localCheck;

	//DRAGONHERE: eventually need to run a last-modified-since check...
	var file : String = GetCacheURL(url);
    if (System.IO.File.Exists(file)) {
	    return "file:///" + file;
    	//DRAGONHERE: http://forum.unity3d.com/threads/70223-WWW-access-to-local-filesystem?p=448574#post448574
    	/*var newFile : String = GetTempCacheURL(url);
    	System.IO.Directory.CreateDirectory(Path.GetDirectoryName(newFile));
...
static function CacheCheck(url : String) {
	#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN
...
function CacheStore(url : String, data : byte[]) {
	#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_EDITOR_OSX
	
	if(CheckLocalURL(url) != "") return;	//This file is already local, we don't want to cache it
	
	file = GetCacheURL(url);
	if (System.IO.File.Exists(file)) return;
	System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(file));
	var fileStream : System.IO.FileStream = new System.IO.FileStream(file, System.IO.FileMode.Create);
	var binaryWriter : System.IO.BinaryWriter = new System.IO.BinaryWriter(fileStream);
...
static function CacheList() {
	...
	//Functional cache
	var cacheDir : System.IO.DirectoryInfo = new System.IO.DirectoryInfo(cachePath);
	//cacheList = cacheDir.GetFileSystemInfos();
	cacheList = System.IO.Directory.GetFiles(cachePath, "*.utw", System.IO.SearchOption.AllDirectories);
	var cacheBytes : float = DirSize(cacheDir) / 1000 / 1000;
	cacheSize = Mathf.RoundToInt(cacheBytes);
	return true;
...
static function DirSize(d : System.IO.DirectoryInfo) : float {
	#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_EDITOR_OSX
	
    var Size : long = 0;    
    var fis : System.IO.FileInfo[] = d.GetFiles();
    for (var fi : System.IO.FileInfo in fis) Size += fi.Length;    
    var dis : System.IO.DirectoryInfo[] = d.GetDirectories();
    for (var di : System.IO.DirectoryInfo in dis) Size += DirSize(di);   
    return Size;
...
static function CacheClear(url : String) {
	#if UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_EDITOR_OSX
	
	try {
		url = url.Replace("\\", "/");
		System.IO.Directory.Delete(System.IO.Path.GetDirectory(url), true);
	}
	catch (e) {
        Debug.Log("The process failed: " + e.ToString());
        return false;
